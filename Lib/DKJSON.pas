...
{$ENDIF !NEXTGEN}
{$ifdef UNICODE}
    class function ParseJSONValue(const Data: AnsiString): TJSONValue; overload; static;
{$endif}
...
begin
  FUtf8offset := 0;
  LDataByte:=FData[FOffset];
  if (LDataByte and (Byte(224))) = Byte(192) then begin
      ...
      FUtf8data[3] := TJSONString.Hex((Byte((LDataByte and Byte(28)))) shr 2);
      FUtf8data[4] := TJSONString.Hex((Byte((Byte(LDataByte) and Byte(3))) shl 2) or (Byte((Byte((FData[FOffset + 1] and Byte(48))) shr 4))));
      FUtf8data[5] := TJSONString.Hex(FData[FOffset + 1] and Byte(15));
      FOffset := FOffset + 2;
    end
  else if (LDataByte and (Byte(240))) = Byte(224) then
    begin
      ...
      FUtf8data[0] := Ord('\');
      FUtf8data[1] := Ord('u');
      FUtf8data[2] := TJSONString.Hex(LDataByte and Byte(15));
  ...
  else if (LDataByte and (Byte(248))) = Byte(240) then
    begin
      if FOffset + 3 >= FRange then
        raise EJSONException.CreateFmt(SUTF8Start, [FOffset]);
      if (FData[FOffset + 1] and (Byte(192))) <> Byte(128) then
...
begin
  ...
  if (FData[FOffset] and (Byte(128))) <> 0 then
     HandleUTF // TG: stack optimization
  else
...
class function TJSONObject.ParseJSONValue(const Data: AnsiString): TJSONValue;
begin
  if Data='' then
     Result:=nil // TG 2015
  else
     Result := ParseJSONValue(BytesOf(Data), 0, True); // Data assumed to be a UTF-8 string
  end;
