unit SHA1Code;
interface
uses Classes, WinApi.Windows, SysUtils;
...
function SHA1DigestToString(const D: TSHA1Digest): UnicodeString;

procedure HashString(s: AnsiString; var Digest: TSHA1Digest);
function SHA1(s:AnsiString):UnicodeString;
function StreamSHA1( Stream : TStream ): AnsiString;
...
begin
  ...
  if bufsize > Stream.Size then
    bufsize:= Stream.Size;
  SetLength(buf,bufsize);
  origbufsize:=bufsize;
  repeat
    bufsize := Stream.Read(buf[0], bufsize);
    SHA1Update( SHA1Context, buf[0], bufsize); // XL allows larger buffer
  until bufsize<origbufsize;
  SHA1Digest := SHA1Final( SHA1Context );
  Result := '';
  for I := 0 to 19 do
   Result :=  Result + AnsiString( IntToHex( SHA1Digest[I], 2 ) );
end;
